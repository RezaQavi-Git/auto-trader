...
def add_technical_indicators(data):
    ...
    data["Signal_Line"] = (
        data["MACD"].ewm(span=signal_window, min_periods=1, adjust=False).mean()
    )
    return data
...
def add_commodity_channel_index(data):
    ...
    data["CCI"] = (typical_price - typical_price.rolling(window=20).mean()) / (
...
def add_williams_percent_r(data):
    ...
    data["Williams %R"] = (
        (highest_high - data["close"]) / (highest_high - lowest_low) * -100
    )
    return data
...
def add_chaikin_oscillator(data):
    adl = (
        (2 * data["close"] - data["low"] - data["high"])
        / (data["high"] - data["low"])
        * data["volume"]
    )
    adl = adl.fillna(0)
    data["Chaikin Oscillator"] = (
        adl.rolling(window=3).mean() - adl.rolling(window=10).mean()
    )
    return data
